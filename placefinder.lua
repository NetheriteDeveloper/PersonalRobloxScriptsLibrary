...
local function CustomNotif(nText, nDuration, nButton1, nButton2)
	StarterGui:SetCore("SendNotification", {
		Title = "Place Finder",
		Text = nText,
		Duration = nDuration or 8,
		Callback = nil,
		Button1 = nButton1,
		Button2 = nButton2
	})
...
if Player:GetAttribute("PlaceFinder_NS") then
	CustomNotif("You already executed this!", 10)
	return
else
	Player:SetAttribute("PlaceFinder_NS", true)
end
...
if Envs.setclipboard == nil then
	Error:Play()
	CustomNotif("Your executor doesn't support setclipboard()", 15)
end
...
for _, v in PlacesPages:GetCurrentPage() do
	...
	FoundPlace.CopyId.MouseButton1Click:Connect(function()
		if Envs.setclipboard then
			...
			Success:Play()
			CustomNotif("Copied Place ID", 8)
		else
			CustomNotif("Your executor doesn't support setclipboard()", 10)
		end
	end)
end
...
CloseB.MouseButton1Click:Once(function()
	PlaceFinder:Destroy()
	Player:SetAttribute("PlaceFinder_NS", nil)
end)
