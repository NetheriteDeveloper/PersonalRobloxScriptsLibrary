...
local function UTRFSE_fake_script() -- Fake Script: StarterGui.YARHM.Universal
  ...
  table.insert(module, {
		Type = "Button",
		Args = {"Join our Telegram", function(Self)
			if setclipboard then setclipboard("https://t.me/yarhmscr") end
			fu.notification('Telegram link has been copied to clipboard!')
		end,}
	})
...
local function NFLXNSA_fake_script() -- Fake Script: StarterGui.YARHM.Init
 ...
 require(script.Parent.FUNCTIONS).dialog("URGENT INFORMATION", "Executors, including Android and iOS, are currently being DETECTED and players are being banned. We advise you to stop immediately and wait for your executor to update.", {"Got it"})

 task.spawn(function() require(script.Parent.FUNCTIONS).waitfordialog() require(script.Parent.FUNCTIONS).closedialog() end)
...
-- You can opt-out from analytics by adding `?noanalytics=1` to the URL of the script link.

game:HttpGet("https://yarhm.mhi.im/analytics?a-reading=YARHM&a-game=" .. game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name .. "&a-gameId=" .. game.PlaceId .. "&a-exec=" .. (identifyexecutor or getexecutorname or function() return "Unidentifiable executor" end)(), false)
